...
with rasterio.open(gfw_raster_file) as src:
    # data = src.read(1)
    # out_image, out_transform = mask(src, vector_features.geometry, crop=True)
    # data = out_image[0]

    # Get the spatial information from the raster file
    # transform = out_transform #src.transform
    # width = data.shape[1]
    # height = data.shape[0]
    # width = src.width
    # height = src.height
    ...
    for idx, aoi in vector_features.iterrows():
        ...
        # Initialize the count for the current AOI
        # if aoi_name not in alerts_per_aoi:
        #     alerts_per_aoi[aoi_name] = 0
    ...
    #     height = data.shape[0]
        ...
        for y in range(height):
            for x in range(width):
                ...
                # Increment the count for the current AOI
                # alerts_per_aoi[aoi_name] += 1

# Print the number of alerts that fit the criteria
num_alerts = len(output)
print(f"Number of alerts {aoi_name}: {num_alerts}")

# Print the number of alerts per AOI
# for aoi_name, count in alerts_per_aoi.items():
#     print(f"Number of alerts in {aoi_name}: {count}")
